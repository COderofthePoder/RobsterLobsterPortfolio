...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between gap-4 py-6 text-left font-display text-lg font-medium transition-colors hover:text-primary [&[data-state=open]>svg]:rotate-45 [&[data-state=open]]:text-primary",
        className,
      )}
      {...props}
    >
